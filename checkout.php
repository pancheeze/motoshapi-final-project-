...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['place_order'])) {
    try {
        ...
        if ($selected_payment_mode === 'paypal') {
            throw new Exception('Please complete payment using the PayPal section below.');
        }
        ...
        if (!$payment_mode_row) {
            $mode_name = $payment_mode === 'paypal' ? 'PayPal' : 'Cash on Delivery (COD)';
            $stmt = $conn->prepare('INSERT INTO payment_modes (mode_name, mode_code, is_active) VALUES (?, ?, 1)');
            $stmt->execute([$mode_name, $payment_mode]);
            ...
            $payment_mode_name = $payment_mode_row['mode_name'] ?: strtoupper($payment_mode);
        }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function setPaypalLoading(isLoading, message) {
        ...
        if (isLoading) paypalLoading.textContent = message || 'Loading PayPal…';
    }
    ...
    const variationId = <?php echo is_null($variation_id) ? 'null' : (int)$variation_id; ?>;
    const quantity = <?php echo (int)$quantity; ?>;
    ...
    if (form) {
        form.addEventListener('submit', function (e) {
            if (selectedPaymentMode() === 'paypal') {
                ...
                togglePaymentUI();
                if (paypalSection) paypalSection.scrollIntoView({ behavior: 'smooth', block: 'start' });
